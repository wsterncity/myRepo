...
AABB::AABB(Point* vertex_list, int num)
{
	min = vertex_list[0];
	max = min;
	for (int i = 1; i < num; i++)
...
AABB::AABB(std::vector<Point>& vertices_list)
{
	min = vertices_list[0];
	max = min;
...
AABBNode::AABBNode(std::vector<Point>& vertexList, std::vector<int>& face_id_list, AABBTreeInfo& treeInfo, int depth)
{
	left = NULL;
	right = NULL;
	m_box = AABB();
...
int AABBNode::FindBestAxis(std::vector<Point>& vertexList)
{
...
void AABBNode::BuildTree(std::vector<Point>& vertexList, std::vector<int>& face_id_list, AABBTreeInfo& treeInfo, int depth)
{
...
AABB_Tree::AABB_Tree(std::vector<Point> &vertices_list)
{
