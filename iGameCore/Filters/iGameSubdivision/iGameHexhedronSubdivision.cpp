...
bool HexhedronSubdivision::Execute() {
  ...
  for (i = 0; i < CellNum; i++) {
    ...
    for (j = 0; j < vcnt; j++) {
      if (j < 4) {
        inPts[i][j] =
            p[j] * 8 + (p[(j + 1) % 4] + p[(j + 3) % 4] + p[j + 4]) * 4 +
            (p[(j + 2) % 4] + p[(j + 1) % 4 + 4] + p[(j + 3) % 4 + 4]) * 2 +
            p[(j + 2) % 4 + 4];
      } else {
        inPts[i][j] =
            p[j] * 8 +
            (p[(j + 1) % 4 + 4] + p[(j - 1) % 4 + 4] + p[j - 4]) * 4 +
            (p[(j + 2) % 4 + 4] + p[(j + 1) % 4] + p[(j - 1) % 4]) * 2 +
            p[(j + 2) % 4];
      }
      inPts[i][j] /= 27;
    }
  ...
  for (i = 0; i < FaceNum; i++) {
    ...
    if (neighborNum == 2) {
      ...
      for (j = 0; j < 4; j++) {
        FacePts[i][j] =
            p[j] * 4 + (p[(j + 1) % 4] + p[(j + 3) % 4]) * 2 + p[(j + 2) % 4];
        FacePts[i][j] /= 9;
      }
    }
  }
  ...
  for (i = 0; i < EdgeNum; i++) {
    ...
    for (j = 0; j < neighborNum; j++) {
      if (tmp.count(cellIds_1[j]))
        cellIds[index++] = cellIds_1[j];
      else
        tmp.insert(cellIds_1[j]);
    }
  ...
  for (cellID = 0; cellID < CellNum; cellID++) {
    ...
    // i边
    igIndex edgeIndexs[4][4] = {
        {0, 1, 0, 0}, {3, 2, 0, 3}, {4, 5, 3, 0}, {7, 6, 3, 3}};
    for (i = 0; i < 4; i++) {
    ...
    // j边
    igIndex edgeIndexs_2[4][4] = {
        {0, 3, 0, 0}, {1, 2, 0, 3}, {4, 7, 3, 0}, {5, 6, 3, 3}};
    for (i = 0; i < 4; i++) {
    ...
    // 内点
    std::array<std::array<int, 3>, 8> interior_map = {{{1, 1, 1},
                                                       {1, 1, 2},
                                                       {1, 2, 2},
                                                       {1, 2, 1},
                                                       {2, 1, 1},
    ...
    //			SubdivisionPts[i][j][k] = BSplineSurface(ControlPts, u,
    //v, w); 			ControlPoints->AddPoint(SubdivisionPts[i][j][k]);
    //		}
    //	}
    // }
    // int size[3] = { n,n,n };
    ...
    //				size[0],1 + size[0],1 + size[0] + size[0] *
    ...
    for (k = 0; k < 4; k++) {
      for (j = 0; j < 4; j++) {
        for (i = 0; i < 4; i++) {
          ...
          // std::cout << k << ' ' << j << ' ' << i << ' ' <<
          // ControlPts[k][j][i][0] << " " << ControlPts[k][j][i][1] << " " <<
          // ControlPts[k][j][i][2] << "\n";
        }
      }
    }
...
double BezierBasis(int i, double t) {
  ...
  case 3:
    return t * t * t;
  default:
    throw std::out_of_range("Index out of bounds for Bezier basis");
  }
}
Point HexhedronSubdivision::BSplineSurface(Point ControlPts[4][4][4], double u,
                                           double v, double w) {
  Point result;
  for (int i = 0; i < 3; ++i) {
    result[i] = 0.0;
    for (int i0 = 0; i0 < 4; ++i0) {
      for (int i1 = 0; i1 < 4; ++i1) {
        for (int i2 = 0; i2 < 4; ++i2) {
          result[i] += ControlPts[i0][i1][i2][i] * BezierBasis(i0, u) *
                       BezierBasis(i1, v) * BezierBasis(i2, w);
        }
      }
    }
  }
