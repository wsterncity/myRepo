...
AABB_Tree(std::vector<Point> &vertices_list);
...
struct  AABB
{
	Point min, max;
	AABB();
	AABB(Point* vertex_list, int num);
	AABB(std::vector<Point>& vertices_list);
	Point getCenter();
};
...
struct AABBNode
{
	...
	AABBNode(std::vector<Point>& vertexList, std::vector<int>& face_id_list, AABBTreeInfo& treeInfo, int depth);
	int FindBestAxis(std::vector<Point>& vertexList);
	void SetBounds(AABB box);
	void BuildTree(std::vector<Point>& vertexList, std::vector<int>& face_id_list, AABBTreeInfo& treeInfo, int depth);
};
...
private:
	...
	int leafNodeNum;
	int all_face_num;
	std::vector<Point> vertices_list_copy;
