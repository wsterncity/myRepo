...
public:
	...
	IGsize GetSize() const {
		return static_cast<IGsize>(m_Offset.size()) - 1;
	}
...
private:
	...
	bool IsEqual(const igIndex* f1, IGsize s1, const igIndex* f2, IGsize s2) const {
		if (s1 != s2) {
			return false;
		}
		if (s1 == 3) {
...
void iGameVolumeMeshClipper::SetExtent(double xMin, double xMax, double yMin, double yMax, double zMin, double zMax, bool flip)
{
...
void iGameVolumeMeshClipper::SetExtent(double boxMin[3], double boxMax[3], bool flip)
{
...
void iGameVolumeMeshClipper::SetPlane(double ox, double oy, double oz, double nx, double ny, double nz, bool flip)
{
...
void iGameVolumeMeshClipper::SetPlane(double orgin[3], double normal[3], bool flip)
{
...
void BuildResult(FacesSet& facesSet, unordered_map<igIndex, int>& numberOfAdjacency,
...
bool iGameVolumeMeshClipper::ExecuteWithVolumeMesh()
{
	...
	auto cellsVisibility = ComputeCellsVisibility(inputMesh->GetPoints(), inputMesh->GetVolumes());
	...
	for (igIndex i = 0; i < cellSize; ++i) {
		...
		unsigned char visible = static_cast<unsigned char>(cellsVisibility->GetValue(i));
		switch (cvnb)
		{
	...
	BuildResult(facesSet, numberOfAdjacency,
		m_OuterMeshVisible, m_InterMeshVisible, m_OuterMeshInvisible);
...
bool iGameVolumeMeshClipper::ExecuteWithUnstructedMesh()
{
	...
	auto cellsVisibility = ComputeCellsVisibility(inputMesh->GetPoints(), inputMesh->GetCells());
...
UnsignedCharArray::Pointer iGameVolumeMeshClipper::ComputeCellsVisibility(Points::Pointer points, CellArray::Pointer cells)
{
	...
	if (!m_BoxClipper.m_Use && !m_PlaneClipper.m_Use) {
		...
		auto computeCellsVisibility = [&](igIndex start, igIndex end) -> void {
			...
			for (igIndex i = start; i < end; ++i) {
				...
				for (igIndex j = 0; j < cvnb; ++j) {
					unsigned char val = static_cast<unsigned char>(pointsVisibility->GetValue(ids[j]));
					if (!val) {
						visibility = false;
						break;
					}
		...
		ThreadPool::parallelFor(0, points->GetNumberOfPoints(), computePointsVisibility);
		ThreadPool::parallelFor(0, cells->GetNumberOfCells(), computeCellsVisibility);
	}
