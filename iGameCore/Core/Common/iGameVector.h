...
template<class ScalarT, int dimension_t>
inline Vector<ScalarT, dimension_t>::Vector(const Scalar& c0, const Scalar& c1, const Scalar& c2)
{
	assert(dimension == 3);
	v_[0] = c0;
	v_[1] = c1;
...
template <class ScalarT, int dimension_t>
Vector<ScalarT, dimension_t> operator*(
	const ScalarT& o, const Vector<ScalarT, dimension_t>& v) {
	return v * o;
}
...
inline std::ostream& operator<<(
	std::ostream& out, const Vector<ScalarT, dimension_t>& vec) {
