...
bool SurfaceMesh::IsBoundaryPoint(const IGsize ptId) {
  ...
  for (int i = 0; i < size; i++) {
    if (this->IsBoundaryEdge(edgeIds[i]))
      return true;
  }
  return false;
}
...
IGsize SurfaceMesh::GetRealMemorySize() {
  ...
  if (m_Faces)
    res += m_Faces->GetRealMemorySize();
  if (m_Edges)
    res += m_Edges->GetRealMemorySize();
  if (m_EdgeDeleteMarker)
    res += m_EdgeDeleteMarker->GetRealMemorySize();
  if (m_FaceDeleteMarker)
