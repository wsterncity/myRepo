...
bool VTKReader::Parsing() {
        ...
        while (true) {
            ...
            if (line[0] == '\0') {
                ...
                if (this->m_FileType == IGAME_BINARY) {
                    ...
                    // Update the dataset
                    if (Types) {
                        this->TransferVtkCellToiGameCell(CellsId, CellsConnect,
                                                         Types);
    ...
    mesh->SetPoints(m_Data.Points);
    auto myTypes = UnsignedIntArray::New();
    myTypes->Resize(ncells);
    auto mytypes = myTypes->RawPointer();
    auto types = Types->RawPointer();
    int type = 0;
    for (int i = 0; i < ncells; i++) {
        type = types[i];
        switch (type) {
            case VTKTYPE::HEXAHEDRON:
                mytypes[i] = IG_HEXAHEDRON;
                break;
            case VTKTYPE::TETRA:
                mytypes[i] = IG_TETRA;
                break;
            case VTKTYPE::PYRAMID:
                mytypes[i] = IG_PYRAMID;
                break;
            case VTKTYPE::WEDGE:
                mytypes[i] = IG_PRISM;
                break;
            case VTKTYPE::POLYHEDRON:
                mytypes[i] = IG_POLYHEDRON;
                break;
            default:
                break;
        }
