...
void SquaredSelection::MousePressEvent(int posX, int posY,
    ...
    mvp_invert = (m_Scene->CameraData().proj_view * m_Scene->ObjectData().model)
                         .invert();
}


void SquaredSelection::MouseMoveEvent(int posX, int posY) {
...
void SquaredSelection::MouseReleaseEvent(int posX, int posY) {
    m_MouseMode = NoButton;
}

void SquaredSelection::WheelEvent(double delta) {}

void SquaredSelection::SelectElement(const std::vector<igm::vec4>& planes) {}

igm::vec4 SquaredSelection::GetPlane(const igm::vec3& p,
                                     const igm::vec3& normal) {
    igm::vec3 n = normal.normalized();
    return igm::vec4(n, -n.dot(p));
}

igm::vec3 SquaredSelection::GetNearWorldCoord(const igm::vec2& screenCoord,
...
igm::vec3 SquaredSelection::GetFarWorldCoord(const igm::vec2& screenCoord,
                                             const igm::mat4& invertedMvp) {
    igm::vec2 NDC(2.0f * screenCoord.x / width - 1.0f,
                  1.0f - (2.0f * screenCoord.y / height));
